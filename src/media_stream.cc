...
{
#if !UVGRTP_HAS_SRTP
    if (rce_flags_ & (RCE_SRTP | RCE_SRTP_KMNGMNT_USER | RCE_SRTP_KMNGMNT_ZRTP |
        RCE_SRTP_REPLAY_PROTECTION | RCE_SRTP_AUTHENTICATE_RTP | RCE_SRTP_KEYSIZE_192 | RCE_SRTP_KEYSIZE_256)) {
        UVG_LOG_ERROR("SRTP support is disabled in this build");
        return RTP_NOT_SUPPORTED;
    }
#endif
#if !UVGRTP_HAS_ZRTP
    if (rce_flags_ & (RCE_ZRTP_DIFFIE_HELLMAN_MODE | RCE_ZRTP_MULTISTREAM_MODE | RCE_SRTP_KMNGMNT_ZRTP)) {
        UVG_LOG_ERROR("ZRTP support is disabled in this build");
        return RTP_NOT_SUPPORTED;
    }
#endif
#if !UVGRTP_HAS_RTCP
    if (rce_flags_ & (RCE_RTCP | RCE_RTCP_MUX)) {
        UVG_LOG_ERROR("RTCP support is disabled in this build");
        return RTP_NOT_SUPPORTED;
    }
#endif
    zrtp_ = zrtp;
...
#if !UVGRTP_HAS_RTCP_READER
            if (!(rce_flags_ & RCE_RTCP_MUX)) {
                UVG_LOG_ERROR("RTCP reader support is disabled in this build");
                return RTP_NOT_SUPPORTED;
            }
#endif
            if (!(rce_flags_ & RCE_RTCP_MUX)) {
...
#if UVGRTP_HAS_SRTP && UVGRTP_HAS_ZRTP
...
#else
rtp_error_t uvgrtp::media_stream::init_srtp_with_zrtp(int, int, std::shared_ptr<uvgrtp::base_srtp>,
    std::shared_ptr<uvgrtp::zrtp>)
{
    return RTP_NOT_SUPPORTED;
}
#endif
